...
The project codifies coding standards in `.github/instructions/` and `.github/copilot-instructions.md`.

- **Comments**: explain intent and decisions (`why`), not mechanics (`what`), and remove stale comments in the same change.
- **Data-layer docs**: every exported function in `db/` and `src/lib/` must include TSDoc/JSDoc with purpose, `@param`, and `@returns` (including injectable `db` argument intent).
- **Astro component contracts**: reusable `.astro` components must document their `Props` interfaces.
- **TypeScript formatting enforcement**: ESLint enforces explicit exported function boundary types for `db/**/*.ts` and `src/lib/*.ts`.
